...
pytest tests/ -v --cov=deepal --cov-report=term-missing
...
git init
git add .
git commit -m "Initial release — deepal v1.0.0"
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/deepal.git
...
```bash
pip install git+https://github.com/YOUR_USERNAME/deepal.git
```
...
- `deepal-1.0.0.tar.gz`  (source distribution)
- `deepal-1.0.0-py3-none-any.whl`  (wheel)
...
# Cell 1 — Install
import subprocess
subprocess.run(['pip', 'install', 'git+https://github.com/YOUR_USERNAME/deepal.git', '-q'])
